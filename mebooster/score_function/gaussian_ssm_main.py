...
import lr_scheduler
from dsm_bk.dsm_loss import anneal_dsm_score_estimation
from gmm import GaussianMixture
from datetime import datetime
from math import pi

from ncsnv2 import get_sigmas, NCSNv2Simple#, ResScore
...
def train_sliced_score(dataload, scorenet1, score_opt, epochs, testx, scheduler, scores_test):#
    scorenet1.train()
    ...
    for epoch in range(epochs): # range(epochs)
        ...
        for data, target in dataload:
            ...
            dsm_loss = sliced_score_estimation_vr_fir(scorenet1, data, n_particles=300)#sliced_score_estimation_vr(scorenet1, data, n_particles=1)
            score_opt.zero_grad()
            dsm_loss.backward()
            score_opt.step()
        if epoch % 1 == 0:
...
def test_scorenet(testset, scorenet1, d, device):
    batch_size=32
    testloader = DataLoader(testset, batch_size, shuffle=False)
    score1 = torch.zeros(len(testset), d).to(device)
    for idx, (x, y) in enumerate(testloader):
        score1[idx * batch_size : min((idx+1) * batch_size, len(testset))] = scorenet1(x)
    return score1
...
def main():
    ...
    # x = torch.load('../adversarial_specialization/data/x_train_20k.pt')
    # y = torch.load('../adversarial_specialization/data/y_train_20k.pt')
    ...
    scores_test = calculate_score_functions_sympy(x_test, gmm_model, len(x_test), d, device)

    # score = get_model(config)
    # s_model1 = torch.nn.DataParallel(score)
    # optimizer = get_optimizer(config, s_model1.parameters())
    scorenet1 = NCSNv2Simple(config).to(device)#NCSNv2Simple
    # scorenet1 = MLP_S1(config).to(device)#DSM_Generator_S1(config).to(device)
    # hidden_units=128
    # scorenet1 = nn.Sequential(
    #     nn.Linear(d, hidden_units),
    #     nn.Softplus(),
    #     nn.Linear(hidden_units, hidden_units),
    #     nn.Softplus(),
    #     nn.Linear(hidden_units, d),
    # ).to(device)

    sigmas = get_sigmas(config)
    # print("sigmas", sigmas)
    labels = torch.randint(0, len(sigmas), (x.shape[0],), device=x.device)
    testset = TransferSetGaussian(x, y)
    s_model1 = test_scorenet(testset, scorenet1, d, device)
    # s_model1 = scorenet1(x)
    print("s_model1.shape", s_model1.shape)
    print("init_model_score vs sympy_score")
    eval_l2_distance(s_model1, scores1)# dim2, a is 4, b is 10

    # print("scores1", scores1[0: 5])
    score_opt = torch.optim.Adam(scorenet1.parameters(), lr=0.00001)
    ...
    scorenet1 = train_sliced_score(dataload, scorenet1, score_opt, config.training.n_epochs, x_test,
                                    scheduler, scores_test) #
    torch.save(scorenet1.state_dict(), './model/scorenet.tar.pth')
