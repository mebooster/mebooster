...
from ncsnv2 import NCSNv2, get_sigmas, NCSNv2Simple, NCSN, DSM_Generator_S1, MLP_S1, NCSNv2SimpleS2, NCSNv2SimpleS3
...
def train_sliced_score3(dataload, scorenet1, score_opt, epochs, testx, scores_test):
    scorenet1.train()
    step=0
    ...
    for epoch in range(epochs): # range(epochs)
        for data, target in dataload:
            step += 1
            dsm_loss = sliced_score_estimation_vr_thir(scorenet1, data, n_particles=1)#sliced_score_estimation_vr(scorenet1, data, n_particles=1)
            score_opt.zero_grad()
            dsm_loss.backward()
            score_opt.step()
        if epoch % 1==0:
...
def main():
    ...
    scores3 = calculate_score_functions_sympy(x, gmm_model, N_query, d, device) #scores2, scores3
    x_test = torch.randn([100, d]).to(device)
    scores_test3 = calculate_score_functions_sympy(x_test, gmm_model, len(x_test), d, device)
    # score = get_model(config)
    # s_model1 = torch.nn.DataParallel(score)
    # optimizer = get_optimizer(config, s_model1.parameters())
    scorenet3 = NCSNv2SimpleS3(config).to(device)
    ...
    eval_l2_distance(s_model3, scores3)

    # print("scores1", scores1[0: 5])
    score_opt = torch.optim.Adam(scorenet3.parameters(), lr=0.0001)
    # scorenet1 = train_denoise_score(dataload, scorenet1, score_opt, config.training.n_epochs, sigmas, x_test, scores_test)
    scorenet3 = train_sliced_score3(dataload, scorenet3, score_opt, config.training.n_epochs, x_test,
                                    scores_test3)
    torch.save(scorenet3.state_dict(), './model/scorenet3.tar.pth')
