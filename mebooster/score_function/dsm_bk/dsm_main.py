...
from ncsnv2 import NCSNv2, get_sigmas, NCSNv2Simple
...
def train_sliced_score(dataload, scorenet1, score_opt, epochs, testx, scores_test):
    scorenet1.train()
    step=0
    ...
    for epoch in range(epochs): # range(epochs)
        for data, target in dataload:
            step += 1
            dsm_loss = sliced_score_estimation_vr(scorenet1, data, n_particles=1)
            score_opt.zero_grad()
            dsm_loss.backward()
            score_opt.step()
        if epoch % 2==0:
...
def main():
    ...
    score_opt = torch.optim.Adam(scorenet1.parameters(), lr=0.001)
