...
from dsm_bk.dsm_main_S2S3 import eval_l2_distance
from gmm import GaussianMixture
from datetime import datetime
from math import pi

from ncsnv2 import NCSNv2, NCSNv2Simple, DSM_Generator_S2, DSM_Generator_S1
...
def main():
    ...
    # x = torch.load('./model/x2.pt').to(device)
    ...
    state_dict = torch.load('./model/scorenet2.tar.pth')
    # scorenet1 = NCSNv2Simple(config).to(device)
    scorenet2 = DSM_Generator_S2(config).to(device)
    scorenet2.load_state_dict(state_dict)
    scorenet1 = DSM_Generator_S1(config).to(device)

    state_dict1 = torch.load('./model/scorenet.tar.pth')
    ...
    len_test = 1000  # len(x)
    x = x[0: len_test]
    scores3 = calculate_score_functions_sympy(x, gmm_model, len_test, d, device) #scores2, scores3 N_query;
    ...
    x.requires_grad = True
    s1_model = scorenet1(x)
    s2_model = scorenet2(x).view(len_test, d, d) #[n_query, d]
