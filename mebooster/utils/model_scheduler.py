...
def train_step(model, train_loader, train_gt_loader=None, criterion=None, optimizer=None, epoch=None, device=None,
    ...
    coh_lambda = 0.3#0.5
    smooth_lambda =0.01
    # for batch_idx, (inputs, (targets, target_labels, point1, vic_u1, vic_gradient1, point2, vic_u2, vic_gradient2)) in enumerate(train_loader):
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        inputs, targets = inputs.to(device), targets.to(device)
        ...
        # loss_values = F.kl_div(outputs, targets, reduction='none').sum()
        # fd_gradient = autograd.grad(loss_values, inputs, create_graph=True)[0]
        # print("fd_gradient", fd_gradient)
        #here has u=[N,m+1,C,S,S]
        # print("u", u.shape)
        epsilon = 1#5e-1
        # victim
        # rand1 = np.random.randint(len(inputs), size=len(inputs)).tolist()
        # rand2 = np.random.randint(len(inputs), size=len(inputs)).tolist()
        # rand3 = np.random.randint(len(inputs), size=len(inputs)).tolist()
        # # print("rand1,", rand1)
        # # print('rand2,', rand2)
        # point1 = inputs[rand1]
        # p_label1 = targets[rand1]
        # point2 = inputs[rand2]
        # p_label2 = targets[rand2]
        # point3 = inputs[rand3]
        # p_label3 = targets[rand3]
        #
        # vic_u1 = point1 - inputs
        # vic_u2 = point2 - inputs
        # vic_u3 = point3 - inputs
        #
        # vic_diff1 =1 / epsilon * (F.kl_div(p_label1, target_labels, reduction='none').sum(dim=1)
        #                     -F.kl_div(targets, target_labels, reduction='none').sum(dim=1)).view(-1, 1, 1, 1)
        # # print("p_label1,", p_label1.shape)
        # # print("targets_labels,", target_labels.shape)
        # vic_input_gradient1 = vic_diff1 * vic_u1
        # vic_input_gradient1 = vic_input_gradient1.flatten(start_dim=1).to(device)
        #
        # vic_diff2 = 1 / epsilon * (F.kl_div(p_label2, target_labels, reduction='none').sum(dim=1)
        #                            - F.kl_div(targets, target_labels, reduction='none').sum(dim=1)).view(-1, 1, 1, 1)
        # vic_input_gradient2 = vic_diff2 * vic_u2
        # vic_input_gradient2 = vic_input_gradient2.flatten(start_dim=1).to(device)
        #
        # vic_diff3 = 1 / epsilon * (F.kl_div(p_label3, target_labels, reduction='none').sum(dim=1)
        #                            - F.kl_div(targets, target_labels, reduction='none').sum(dim=1)).view(-1, 1, 1, 1)
        # vic_input_gradient3 = vic_diff3 * vic_u3
        # vic_input_gradient3 = vic_input_gradient3.flatten(start_dim=1).to(device)
        point1, vic_u1, vic_gradient1, point2, vic_u2, vic_gradient2 = point1.to(device), vic_u1.to(device), \
                                                                       vic_gradient1.to(device), point2.to(device), \
                                                                       vic_u2.to(device), vic_gradient2.to(device)
        vic_gradient1 = vic_gradient1.flatten(start_dim=1)
        vic_gradient2 = vic_gradient2.flatten(start_dim=1)
        #copy model
        # evaluation_point_0 = inputs + epsilon * u  # u is random unit vector
        evaluation_point_0 = point1#evaluation_point_0.to(device)
        pred_0 = F.softmax(model(evaluation_point_0), dim=1)

        diff_0 = 1 / epsilon * (F.kl_div(pred_0, target_labels, reduction='none').sum(dim=1)
                            -F.kl_div(y_pred, target_labels, reduction='none').sum(dim=1)).view(-1, 1, 1, 1)
        fd_gradient = diff_0 * vic_u1

        evaluation_point_2 = point2#evaluation_point_2.to(device)
        pred_2 = F.softmax(model(evaluation_point_2), dim=1)
        diff_2 = 1 / epsilon * (F.kl_div(pred_2, target_labels, reduction='none').sum(dim=1)
                                - F.kl_div(y_pred, target_labels, reduction='none').sum(dim=1)).view(-1, 1, 1, 1)
        fd_gradient_2 = diff_2 * vic_u2

        fd_gradient_2 = fd_gradient_2.flatten(start_dim=1)
        fd_gradient = fd_gradient.flatten(start_dim=1)

        # cos_sim = F.cosine_similarity(fd_gradient, vic_input_gradient, dim=-1)
        # coh_loss = torch.log(cos_sim.exp().sum(dim=-1) + log_offset).mean()

        # print("coh_loss", coh_loss)
        # print("targets", targets)
        ...
        #
        # pred_entropy = torch.sum(-torch.mul(ensemble_probs, torch.log(ensemble_probs + 1)),
        #                              dim=-1).mean()
        # bool_R_y_true = torch.eq(torch.ones_like(target_labels) - target_labels,
        #                          torch.ones_like(target_labels))
        # mask_non_y_pred = []
        # mask_non_y_pred.append(torch.masked_select(y_pred, bool_R_y_true).reshape(-1, 10 - 1))
        # mask_non_y_pred.append(torch.masked_select(targets, bool_R_y_true).reshape(-1, 10 - 1))
        ...
        #     matrix + det_offset * torch.eye(2, device=matrix.device).unsqueeze(0)).mean()

        # print("fd_gradient.size,", fd_gradient.shape, ",", vic_gradient1.shape)
        #previously: criterion(outputs, targets)
        loss = criterion(outputs, targets)# + smooth_lambda * torch.norm(fd_gradient - vic_gradient1, p=2)\
               #+ smooth_lambda * torch.norm(fd_gradient_2 - vic_gradient1, p=2)
               #+ smooth_lambda * torch.norm(fd_gradient_3 - vic_input_gradient3, p=2)

        # loss = criterion(outputs, targets) - coh_lambda * coh_loss - smooth_lambda * torch.norm(fd_gradient, p=2)

        # loss = criterion(outputs, target_labels) - 0.5 / log_det# pred_entropy +
