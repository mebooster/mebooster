...
def train_step(model, train_loader, train_gt_loader=None, criterion=None, optimizer=None, epoch=None, device=None,
    ...
    coh_lambda = 0.3#0.5
    smooth_lambda =0.01
    for batch_idx, (inputs, (targets, evaluation_point_0, vic_input_gradient, u, target_labels
                             , evaluation_point_2, vic_input_gradient_2, u_2)) in enumerate(train_loader):
        inputs, targets = inputs.to(device), targets.to(device)
        inputs.requires_grad = True
        vic_input_gradient = vic_input_gradient.to(device)
        u = u.to(device)
        u_2 = u_2.to(device)
        ...
        evaluation_point_0 = evaluation_point_0.to(device)
        pred_0 = F.softmax(model(evaluation_point_0), dim=1)
        # evaluation_point_1 = inputs - epsilon * u  # u is random unit vector
        # pred_1 = F.softmax(model(evaluation_point_1), dim=1)
        ...
        fd_gradient = diff_0 * u

        evaluation_point_2 = evaluation_point_2.to(device)
        ...
        vic_input_gradient = vic_input_gradient.flatten(start_dim=1).to(device)
        ...
        loss = criterion(outputs, targets) + smooth_lambda * torch.norm(fd_gradient - vic_input_gradient, p=2)\
               + smooth_lambda * torch.norm(fd_gradient_2 - vic_input_gradient, p=2)
