...
def main():
    ...
    sm_set = ['mnist_tl,,ini,rec', 'mnist_tl,,over,rec'] #'adversarial_deepfool,,over,rec' 'adversarial_deepfool,,over', 'adversarial_deepfool,,base',
    for sm_m in sm_set:
        start_time = []
        ...
        if params['strategy'] == 'base':
            attack_model_name = 'lenet_tl'
        else:
            attack_model_name = 'over_lenet_tl'
        ...
        if params['iterative']:  # start iteration train
            for it in range(params['num_iter']):
                ...
                attack_model, _ = Blackbox.from_modeldir_split_attack_mode(out_path, 'checkpoint_{}_{}.pth.tar'.format(b, params['strategy']), device) #resume
                ...
                #     nc = 1
                #     init_w1 = over_initial_cnn(w1_dir1, w1_dir2, w1_dir3, w1_dir4, w1_dir5, channel=nc).to(device)
