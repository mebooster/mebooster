    ...
    def get_transferset(self, k, sampling_method='random', ma_method='unsupervised', pre_idxset=[],
        ...
        with tqdm(total=k) as pbar:
            if self.sampling_method == 'initial_seed' or self.sampling_method == 'training_data' or\
                ...
                chosed_idx = [list(self.idx_set)[int(e)] for e in initial_seed]  # idx_set-chosed_idx
...
def main():
    ...
    blackbox, num_classes = Blackbox.from_modeldir_split(blackbox_dir, device)  # get victim_model
    blackbox.eval()
    ...
    testset = test_dataset(train=False, transform=test_transform)  #download
        ...
        if params['iterative']:  # start iteration train
            for it in range(params['num_iter']):
                ...
                if (it + 1) < params['num_iter']:
                    ...
                    adversary.set_attack_model(attack_model, device)  #attack model
