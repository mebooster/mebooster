...
from utils import clipDataTopX
import time
import datetime
            ...
            if self.sampling_method == 'initial_seed' or self.sampling_method == 'training_data' or\
                ...
                if sampling_method == 'kcenter': #4
                    ...
                    if self.ma_method == 'unsupervised':
                        ...
                        sss1 = UnsupervisedMemberAttack(size=k, Y_vec=Y_copy, unsuperY=self.unsuperY)  # Y from copy
                        s1 = sss1.get_subset()
                        ...
                        print("\nuse shadow_model mia to choose and query data")
                        sss1 = AdjShadowModelMemberAttack(size=k, shadow_attack_model=shadow_attack_model,
                                                       Y_vec=Y_copy, batch_size=10, transfery=self.transfery, it=it)
                        s1, choosen_idx_10, conf_idx_10 = sss1.get_subset()
                        # training_set = []
                        # for l in range(len(conf_idx_10)):
                        #     temp_set = [list(self.idx_set)[e] for e in np.asarray(choosen_idx_10[l]).astype(int)]
                        #     training_set.append(temp_set)
                    else:
                        print("\nwrong sampling_method/membership inference setting")

                    # second sample select strategy
                    if second_sss == 'random':
                        sss = RandomSelectionStrategy(k, Y_copy, previous_s=s1)
                    elif second_sss == 'margin':  # 2
                        sss = MarginSelectionStrategy(k, Y_copy, previous_s=s1)
                    elif second_sss == 'uncertainty':  # 1
                        sss = UncertaintySelectionStrategy(k, Y_copy, previous_s=s1)
                    elif second_sss == 'graph_density':  # 3
                        sss = GraphDensitySelectionStrategy(k, Y_copy, self.get_initial_centers(), previous_s=s1)
                    elif second_sss == 'adversarial_deepfool':  # 4
                        sss = AdversarialDeepFoolStrategy(size=k, X=X_rest, Y_vec=Y_copy, copy_model=self.attack_model,
                                                          previous_s=s1)
                    elif second_sss == 'kcenter':  # 5
                        sss = KCenterGreedyApproach(k, Y_copy, self.get_initial_centers(), previous_s=s1)  # Y_copy
                    elif second_sss == 'bayes_ald_dropout':  # 6
                        sss = BALDDropoutStrategy(k, Y_copy, previous_s=s1)


                elif self.sampling_method == 'upperbound':
                    print("self.no_training_in_initial:", self.no_training_in_initial)
                    if second_sss == 'uncertainty':  # 1
                        sss = UncertaintySelectionStrategy(k, Y_copy[:int(
                            cfg.trainingbound - len(self.pre_idxset) + self.no_training_in_initial)])
                        print("use upperbound and uncertainty")
                    elif second_sss == 'margin':  # 2
                        sss = MarginSelectionStrategy(k, Y_copy[:int(
                            cfg.trainingbound - len(self.pre_idxset) + self.no_training_in_initial)])
                        print("use upperbound and margin")
                    elif second_sss == 'graph_density':  # 3
                        sss = GraphDensitySelectionStrategy(k, Y_copy[:int(
                            cfg.trainingbound - len(self.pre_idxset) + self.no_training_in_initial)]
                                                            , self.get_initial_centers())
                        print("use upperbound and graph_density")
                    elif second_sss == 'adversarial_deepfool':  # 5
                        sss = AdversarialDeepFoolStrategy(size=k, X=X_rest, Y_vec=Y_copy[:int(
                            cfg.trainingbound - len(self.pre_idxset) + self.no_training_in_initial)]
                                                          , copy_model=self.attack_model)
                        print("use upperbound and adversarial_deepfool")
                    elif second_sss == 'kcenter':  # 4
                        sss = KCenterGreedyApproach(k, Y_copy[:int(
                            cfg.trainingbound - len(self.pre_idxset) + self.no_training_in_initial)],
                                                    self.get_initial_centers())
                        print("use upperbound and kcenter")
                    elif second_sss == 'bayes_ald_dropout':  # 6
                        sss = BALDDropoutStrategy(k, Y_copy[:int(
                            cfg.trainingbound - len(self.pre_idxset) + self.no_training_in_initial)])
                        print("use upperbound and bayes_ald_dropout")

                # if it <= cfg.num_iter:
                #     s = sss.get_subset()
                # else:
                #     s1 = np.zeros([len(choosen_idx_10)]).tolist()
                #     s=[]
                #     for i in range(len(choosen_idx_10)):
                #         s1[i] = sss[i].get_subset()
                #         s.append(s1[i])
                ...
                for b in range(int(np.ceil(len(self.pre_idxset) / self.batch_size))):  # s
                    # x_b = x_t[b * self.batch_size: min((1 + b) * self.batch_size, len(s))].to(self.blackbox.device)
                    c_idx = self.pre_idxset[b * self.batch_size: min((1 + b) * self.batch_size, len(self.pre_idxset))]  # chosed_idx
...
def train_bi_classifier(shadow_model, shadow_set, shadow_set_out, batch_size=10, num_workers=10, device = 'cuda', shadow_model_path='data'):
    # Data loaders
...
def main():
    ...
    sm_set = ['mnist_tl,,ini', 'mnist_tl,,base'] #'mnist_tl,,over',
    for sm_m in sm_set:
        start_time = []
