...
from ncsnv2 import NCSNv2Simple, get_sigmas, NCSNv2SimpleS2, Score
...
# def train_vae_model(train_loader, d, N_query, batch_size, device, vae_cfg):
#     start_time = time.time()
#
#     train_ds = cycle(train_loader)
#     vae_model = Model(latent_size=vae_cfg.latent_size, data_size=vae_cfg.data_size)
#     if vae_cfg.variational == "flow":
#         variational = VariationalFlow(
#             latent_size=vae_cfg.latent_size,
#             data_size=vae_cfg.data_size,
#             flow_depth=vae_cfg.flow_depth,
#         )
#     elif vae_cfg.variational == "mean-field":
#         variational = VariationalMeanField(
#             latent_size=vae_cfg.latent_size, data_size=vae_cfg.data_size
#         )
#     else:
#         raise ValueError(
#             "Variational distribution not implemented: %s" % vae_cfg.variational
#         )
#
#     vae_model.to(device)
...
#         list(vae_model.parameters()) + list(variational.parameters()),
...
#     t0 = time.time()
#
#     for step in range(vae_cfg.max_iterations):
#         batch = next(train_ds)
#         x = batch[0].to(device)
#         vae_model.zero_grad()
...
#         optimizer.step()
#
#         if step % vae_cfg.log_interval == 0:
#             t1 = time.time()
#             examples_per_sec = vae_cfg.log_interval * batch_size / (t1 - t0)
#             with torch.no_grad():
#                 valid_elbo, valid_log_p_x = evaluate(
#                     vae_cfg.n_samples, vae_model, variational, train_loader
#                 )
#             print(
#                 f"Step {step:<10d}\t"
...
#                 }
#                 torch.save(states, vae_cfg.train_dir / "vae_best_state_dict")
#             t0 = t1
#
#     checkpoint = torch.load(vae_cfg.train_dir / "vae_best_state_dict")
#     vae_model.load_state_dict(checkpoint["model"])
#     variational.load_state_dict(checkpoint["variational"])
#     print(f"Total time: {(time.time() - start_time) / 60:.2f} minutes")
#
#     return vae_model, variational
...
def main():
    ...
    X_train = torch.load("data\\x_train_60000.pt").to(device)
    ...
    y = torch.load('data\\y_train_60000.pt').to(device)
    ...
    scorenet1 = Score(config).to(device)  #NCSNv2Simple DSM_Generator_S1
    state_dict1 = torch.load('./model/scorenet_step1.tar.pth')
    scorenet1.load_state_dict(state_dict1)

    #####
    M2, T = calculate_score_functions_via_model(step_1_x.to(device), y.to(device), scorenet1, scorenet2, device) #X_train
    # print("M2", M2.shape)
    # print("T", T.shape)
    ...
    # d x m1
    # err = dirErr(teacher_model, W1_dir)
    # print("error", err)
    #
    ...
    # eval_hid_direction(W1_dir, teacher_model, device)

    # print("W1_dir2")
    # eval_direction(W1_dir2, teacher_model, device)

    # print("eval nn")
    # student_model.fc1.weight = Parameter(W1_dir.T.cpu().detach().to(device))
    # eval_model(X_train, teacher_model, student_model, random_model)
