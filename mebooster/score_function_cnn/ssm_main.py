...
from ncsnv2 import NCSNv2, get_sigmas, NCSNv2Simple, NCSN, DSM_Generator_S1, MLP_S1, Score
...
def train_sliced_score(dataload, scorenet1, score_opt, epochs, testx, scores_test, scheduler):#scores_test,
    scorenet1.train()
    ...
    for epoch in range(epochs): # range(epochs)
        ...
        for data, target in dataload:
            ...
            dsm_loss = sliced_score_estimation_vr_fir(scorenet1, data, n_particles=50)#sliced_score_estimation_vr(scorenet1, data, n_particles=1)
            score_opt.zero_grad()
            dsm_loss.backward()
            score_opt.step()
        if epoch % 1 == 0:
...
def main():
    ...
    x = torch.load('data\\step_1_x.pt')
    y = torch.load('data\\y_train_60000.pt').to(device)
    #y = torch.load('../adversarial_score_initial/ideal_cnn_data/y_train_20k.pt')
    N_query = x.shape[0]
    d = x.shape[1]*x.shape[2]*x.shape[3]
    x = x.reshape(N_query, d).to(device)
    #y = torch.randn([N_query, 1]).to(device)
    ...
    config.data.dim = d
    scorenet1 = Score(config).to(device)#NCSNv2Simple
    ...
    score_opt = torch.optim.Adam(scorenet1.parameters(), lr=0.000005)
    scheduler = lr_scheduler.lr_scheduler(mode='cos',
                                          init_lr=0.000005,
                                          num_epochs=100,
                                          iters_per_epoch=len(dataload),
                                          lr_milestones=[15, 30, 45, 60, 75],
    ...
    scorenet1 = train_sliced_score(dataload, scorenet1, score_opt, config.training.n_epochs, x_test, scores_test,
                                    scheduler) #scores_test
    torch.save(scorenet1.state_dict(), './model/scorenet_step1.tar.pth')
