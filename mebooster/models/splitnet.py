...
class SplitNet(nn.Module):
	def __init__(self,
		...
		# using cot_loss (co-diff_training loss)
	...
	def forward(self, x, target=None, mode='train', epoch=0, streams=None):
		...
		if 'train' in mode:
			...
			# Only do mixup for one time, otherwise it will violate the co-diff_training loss and ensembled loss
			# if self.is_mixup:
	...
	def _co_training_loss(self, outputs, loss_choose, epoch=0):
		"""calculate the co-diff_training loss between outputs of different small networks
		"""
